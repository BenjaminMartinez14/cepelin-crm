-- 0012_fix_sow_percentage.sql
-- Fix sow_percentage to only use active invoice statuses.
-- Denominator now includes cedida_mx; returns NULL (→ "—" in UI)
-- when no active invoices exist at all.
...
LEFT JOIN LATERAL (
  SELECT
    ...
    -- Share of Wallet: Xepelin active / all active (including cedida_mx).
    -- NULL when no active invoices exist → shows "—" in UI.
    round(
      100.0 * coalesce(sum(i.amount) FILTER (
        WHERE i.status IN ('cedida_xepelin', 'en_cobranza')
      ), 0)
      / nullif(sum(i.amount) FILTER (
        WHERE i.status IN ('cedida_xepelin', 'en_cobranza', 'cedida_competencia', 'cedida_mx')
      ), 0),
      1
    ) AS sow_percentage
  FROM invoices i
