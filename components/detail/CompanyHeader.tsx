...
import type { CompanyMetrics, Contact, Gestion, Note } from "@/types";
...
export function CompanyHeader({
  ...
  onGestionAdded?: (gestion: Gestion) => void;
}) {
  ...
  async function save() {
    ...
    try {
      ...
      const res = await apiPatch<{ next_followup_date: string | null }>(`/api/companies/${company.id}/followup`, { date });
  ...
  return (
    <>
        ...
        <CardContent className="flex flex-col gap-4 pt-6 sm:flex-row sm:items-start sm:justify-between">
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground">
              {taxIdLabel(company.country)} {company.tax_id} · Enrolada {formatDate(company.enrolled_at)}
            </p>
            {contact && (
              <p className="text-sm text-muted-foreground">
          ...
          <div className="flex flex-col items-end gap-3">
            <Button onClick={() => setGestionOpen(true)}>+ Registrar gestión</Button>
            <div className="text-sm text-right">
              <p className="text-xs uppercase tracking-wide text-muted-foreground">Próximo seguimiento</p>
              {editing ? (
                <div className="mt-1 flex items-center gap-2">
                  <Input type="date" value={draft} onChange={(e) => setDraft(e.target.value)} className="h-8 w-40" />
                  <Button size="sm" onClick={save} disabled={saving}>{saving ? "…" : "Guardar"}</Button>
                  <Button size="sm" variant="ghost" onClick={() => setEditing(false)}>Cancelar</Button>
                </div>
              ) : (
                <button className="mt-1 font-medium underline-offset-4 hover:underline" onClick={() => { setDraft(followup ?? ""); setEditing(true); }}>
      ...
      <GestionModal companyId={company.id} open={gestionOpen} onOpenChange={setGestionOpen} onSuccess={(g) => onGestionAdded?.(g)} />
    </>
  );
}
