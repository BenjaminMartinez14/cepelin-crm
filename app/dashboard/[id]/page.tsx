...
import { GestionesSection } from "@/components/detail/GestionesSection";
import { NotesSection } from "@/components/detail/NotesSection";
import { apiGet } from "@/lib/api";
import type { CompanyDetail, Gestion, Note } from "@/types";

export default function CompanyDetailPage({ params }: { params: { id: string } }) {
  const [detail, setDetail] = useState<CompanyDetail | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [pendingNote, setPendingNote] = useState<Note | null>(null);
  const [pendingGestion, setPendingGestion] = useState<Gestion | null>(null);

  const handleNoteAdded = useCallback((note: Note) => setPendingNote(note), []);
  const handleGestionAdded = useCallback((gestion: Gestion) => {
    setPendingGestion(gestion);
    apiGet<CompanyDetail>(`/api/companies/${params.id}`).then(setDetail).catch(() => {});
  }, [params.id]);
  ...
  return (
    <div className="space-y-6">
      <Link href="/dashboard" className="text-sm text-muted-foreground hover:underline">← Volver a mi cartera</Link>
      ...
      {detail && (
        <>
          <CompanyHeader company={detail.company} contacts={detail.contacts} onNoteAdded={handleNoteAdded} onGestionAdded={handleGestionAdded} />
          <MetricsRow company={detail.company} />
          <WhatsappSummaryCard company={detail.company} />
          {detail.company.interaction_summary && (
            <Card className="p-5">
              <h3 className="text-sm font-semibold uppercase tracking-wide text-muted-foreground mb-3">Última interacción KAM</h3>
              <p className="text-sm leading-relaxed text-foreground whitespace-pre-wrap">{detail.company.interaction_summary}</p>
            </Card>
          )}
          <AiAnalysis company={detail.company} />
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <div className="lg:col-span-2">
              <VolumeChart data={detail.monthly_volume} country={detail.company.country} />
            </div>
          ...
          <GestionesSection companyId={detail.company.id} pendingGestion={pendingGestion} />
